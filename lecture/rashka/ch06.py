...
X = df.loc[:, 2:].values
y = df.loc[:, 1].values
le = LabelEncoder() #Convert the M and B values into integers
y = le.fit_transform(y)
print("le.classes_", le.classes_)
print("le.transform", le.transform(['M', 'B']))
...
#Sci-Kit Learn has a cross val scorer function which allows us to get the same results with less code
scores = cross_val_score(estimator=pipe_lr,
                         X=X_train,
                         y=y_train,
                         cv=10,
                         n_jobs=1) #n_jobs lets us use multiple core on our computer
